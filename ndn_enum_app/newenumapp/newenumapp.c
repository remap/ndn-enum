/**
* This file newenumapp.c attempts to enumerate all the names under a given name prefix.
* It is used for applications to pull everything in a branch of the NDN name hierarchy.
* Partially adapted from ccnslurp.c
*
*
*
*
...
// From lightData.c, get string element for each interest component;
...
//    data -> excl = realloc(data -> excl, (data->n_excl+count) * sizeof(data -> excl[0]));
//    data -> excl[data->n_excl] = comp;  // Problem ???
//    data -> n_excl = data -> n_excl + count;
    // ***************
    ...
    if (matched_comps + 1 == comps -> n) {
    	/* Reconstruct the implicit ContentObject digest component */
    	ccn_digest_ContentObject(ccnb, info -> pco);
    	ccn_name_append(comps, info->pco->digest, info->pco->digest_bytes);
    	data -> excl = realloc(data->excl, (data -> n_excl + 1) * sizeof(data->excl[0]));
    	data -> excl[data -> n_excl++] = comp;
    }
    else {
    	...
    	int j = 0;
    	char *str;
    	count = comps -> n - matched_comps - 1;
//*    	printf("matched_comps = %d, comps->n = %d\n", matched_comps, comps -> n);

    	// component counts from 0; e.g. /ndn/app/files/001/002, matched_comps = 3,
    	// but comps->buf[matched_comps] = 001. count = 6, maybe 5 compnents plus one digest.
 //*   	for (j = 0; j <= count-1; j++) {
 //*   		str = ccn_name_comp_to_str(info->content_ccnb, info->content_comps, matched_comps + j);
 //*   		printf("The %d th component of the content is %s\n", j, str);
 //*   	}

    	while (count) {
//*    		printf("*** add = %d, count = %d\n", add, count);
    		ccn_name_append_components(comp, ccnb, comps->buf[matched_comps + add], comps->buf[matched_comps+add+1]);
    		data->excl = realloc(data->excl, (data->n_excl + 1) * sizeof(data->excl[0]));
    		data->excl[data->n_excl++] = comp;
    		comp = NULL;
    		comp = ccn_charbuf_create();
...
// Function for timing program execution time
...
//	(void) time(&t1);
    ...
    for (i = 0;; i++) {
        n = counter;
        ccn_run(ccn, 10); /* stop if we run dry for 1 sec == 1000*/
        fflush(stdout);
        if (counter == n)
            break;
    }
